...
class ViolinDataset():
  def __init__(self, 
    ...
    self.midi_pth, self.perform = get_midi_pth(midi_pth, composers)
  ...
  def random_chunk(self, midi_end_time):
  ...
  def __getitem__(self, idx):
    if self.on_memory:
      ...
      audio_start, audio_end = self.random_chunk(self.midi_end_time_list[idx])
      ...
      audio_start, audio_end = self.random_chunk(midi.get_end_time())  
    ...
    return chunk_pitch, chunk_onset, chunk_bend, chunk_velocity, performer, chunk_offset
...
class ValidViolinDataset(ViolinDataset):
  def __init__(self, 
    ...
    self.midi_pth, self.perform = get_midi_pth(valid_pth, composers)
    ...
    self.midi_roll_list = [self.preprocess_midi(midi) for midi in (tqdm(self.midi_list))]
    print('All data loaded to memory')
    self.chunk_pitch, self.chunk_onset, self.chunk_bend_list, self.chunk_vel_list, self.chunk_perf, self.chunk_offset, self.chunk_sec = self.chunk_all_audio()
  ...
  def chunk_all_audio(self):
    ...
    for i in tqdm(range(len(self.midi_list))):
      midi_roll = self.midi_roll_list[i]
      midi_data = self.midi_list[i]
      perforormer = self.perform[i]
        
      chunk_pitch, chunk_onset, chunk_bend, chunk_velocity, chunk_perform_list, chunk_offset_list, chunk_sec = self.chunk_data_roll(midi_data, midi_roll, perforormer, self.segment_size)
    ...
    return all_chunk_pitch, all_chunk_onset, all_chunk_bend, all_chunk_vel, all_chunk_perf, all_chunk_offset, all_chunk_sec


  def chunk_data_roll(self, midi_data, mid_roll, perforormer, segment_size):
    chunk_pitch_list = []
    chunk_onset_list = []
    chunk_bend_list = []
    chunk_perform_list = []
    ...
    return chunk_pitch_list, chunk_onset_list, chunk_bend_list, chunk_velocity_list, chunk_perform_list, chunk_offset_list, chunk_sec

  def __len__(self):
    return len(self.chunk_pitch)
  ...
  def __getitem__(self, idx):
    ...
    return self.chunk_pitch[idx], self.chunk_onset[idx], self.chunk_bend_list[idx], self.chunk_vel_list[idx], performer, self.chunk_offset[idx]